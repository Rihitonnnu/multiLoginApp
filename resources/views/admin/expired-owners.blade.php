                                        ...
                                        @foreach ($expiredOwners as $owner)
                                            <tr>
                                                ...
                                                <form id="delete_{{$owner->id}}" method="post" action="{{route('admin.expired-owners.destroy',['owner'=>$owner->id])}}">
                                                    @csrf
                                                    <td class="px-4 py-3">
                                                        <a href="#" data-id="{{$owner->id}}"" onclick="deletePost(this)" class="flex mx-auto text-white bg-red-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-lg">完全に削除</a>
                                                    </td>
                                                </form>
                                            </tr>
                                        @endforeach
